...
COPY packages/syncan-app/package*.json ./packages/syncan-app/
...
# Copy backend code
COPY --from=build /app/packages/syncan-backend /app/packages/syncan-backend

# Copy the built React app to Nginx's web server directory
COPY --from=build /app/packages/syncan-app/build /usr/share/nginx/html
